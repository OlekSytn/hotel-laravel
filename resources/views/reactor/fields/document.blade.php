...
<div class="form-group-column form-group-column--{{ array_get($options, 'fullWidth', false) ? 'full' : 'field' }} ">
    ...
    <div class="form-group__document-container {{ ($document = get_nuclear_documents(request()->old($name, null) ?: $options['value'])) ? '' : ' empty' }}"
         data-filter="{{ array_get($options, 'filter', 'all') }}">
        @if($document)
            <figure class="form-group__document" data-id="{{ $document->getKey() }}" data-type="{{ $document->type }}"
                    data-summary="{{ $document->summarize(true) }}">
                <span>{!! $document->present()->thumbnail !!}</span>
                <figcaption class="form-group__document-title">{{ $document->name }}</figcaption>
            </figure>
        @endif
