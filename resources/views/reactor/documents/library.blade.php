            ...
            <div class="library__columns">
                <div class="library-column library-column--documents">
                    ...
                    <div class="library-column__toolbar">
                        ...
                        <form class="header__search library-column__tool" method="GET"
                              action="{{ route('reactor.documents.search.json') }}" id="librarySearch">
                            <i class="icon-search"></i>
                            <input type="search" name="q" placeholder="{{ trans('documents.search') }}..."
                                   autocomplete="off">
                        </form>
                    </div>
                ...
                <div class="library-column library-column--details">
                        ...
                        <div class="library-details library-details--empty">
                            <div class="library-details__content">
                                ...
                                <div class="library-details__translations" id="libraryTranslations">
                                    @if(locale_count() > 1)
                                        <ul class="compact-tabs">
                                            ...
                                            @foreach(locales() as $locale)
                                                <li class="compact-tabs__tab compact-tabs__tab--{{ $locale }}{{ $i === 0 ? ' compact-tabs__tab--active' : '' }}"
                                                    data-locale="{{ $locale }}">
                                                    {{ uppercase($locale) }}</li>
                                                <?php $i++; ?>
                                            @endforeach
                                    ...
                                    <form id="libraryTranslationsForm" method="POST"
                                          action="{{ route('reactor.documents.update.json') }}">
                                        <input type="hidden" name="document" val="">

                                        <div class="sub-tabs">
                                            <?php $i = 0; ?>
                                            @foreach(locales() as $locale)
                                                <div class="sub-tab sub-tab--{{$locale}}{{ $i === 0 ? ' sub-tab--active' : '' }}"
                                                     data-locale="{{ $locale }}">
